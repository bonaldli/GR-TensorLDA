...
from module_GR_TensorLDA_OnlineLearning import GR_OnlineTensorLDA
...
save_dir = 'C:/Users/zlibn/Desktop/save_results_online'
...
likelihood_evolu_e = np.zeros((num_epoch, 2))

# perO_evolu_i = np.zeros((num_batch, 2))
# perD_evolu_i = np.zeros((num_batch, 2))
# perT_evolu_i = np.zeros((num_batch, 2))

perO_evolu_e = np.zeros((num_epoch, 2))
perD_evolu_e = np.zeros((num_epoch, 2))
perT_evolu_e = np.zeros((num_epoch, 2))
...
for lam_k, lam_v in enumerate(LAMBDA):
    ...
    for e in range(num_epoch):
        ...
        _updatect = e*S
        model = GR_OnlineTensorLDA(worker_idx, alpha, betaO, betaD, betaT, num_station, num_time, J, K, L, M, S, \
        ...
        for i in range(num_batch):
            ...
            count_uzo, count_uzd, count_uzt, betaO, betaD, betaT, gamma, theta, likelihood_mb, time_lkh \
                = model.fit(i=i, docs_minibatch=docs_minibatch, lam=lam_v, mu=mu, nu=nu, G_net=G_net, G_poi=G_poi, \
                            dictionary_o=dictionary_o, dictionary_d=dictionary_d, dictionary_t=dictionary_t, idx_corpus_o=idx_corpus_o, \
                                idx_corpus_d=idx_corpus_d, idx_corpus_t=idx_corpus_t, num_user=num_user, num_station=num_station, num_time=num_time, betaO=betaO, betaD=betaD, betaT=betaT)
    ...
    np.save(save_dir+'/betaO'+'_lam'+f'_{lam_v}'+'_.npy', betaO_o)
    #np.save(save_dir+'/gradientO'+'_lam'+f'_{lam_v}'+'_.npy', gradientO)
    #np.save(save_dir+'/hessianO'+'_lam'+f'_{lam_v}'+'_.npy', hessianO)
    
    np.save(save_dir+'/betaD'+'_lam'+f'_{lam_v}'+'_.npy', betaD_o)
    np.save(save_dir+'/betaT'+'_lam'+f'_{lam_v}'+'_.npy', betaT_o)
