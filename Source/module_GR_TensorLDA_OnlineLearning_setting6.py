...
class GR_OnlineTensorLDA:
    ...
    def fit(self, i, docs_minibatch, lam, mu, nu, G_net, G_poi, dictionary_o, dictionary_d, dictionary_t, idx_corpus_o, idx_corpus_d, idx_corpus_t, num_user, num_station, num_time, betaO, betaD, betaT, gamma, theta):
        """ 
    ...
    #for i_em in range(iterEM):
        ...
        for s in range(S):
            ...
            phiO, phiD, phiT, gamma, likelihood_s = self.variationalInference(i, docs_minibatch, s, u, gamma, phiO, phiD, phiT, betaO, betaD, betaT, idx_corpus_o, idx_corpus_d, idx_corpus_t)
            ...
            for j in range(J):
                for k in range(K):
                    for l in range(L):
                        theta[u, j, k, l] = sum( phiO[w, j] * phiD[w, k] * phiT[w, l] for w in range(int(docs_minibatch.iloc[s]['wordcount'])) )
        ...
        betaO_tilde, gradientO, hessianO = self.update_beta_w_graph(num_batch, num_station, lam, betaO_no_g, mu, G_net, G_poi, NT_CONVERGED, g_step)
        ...
        betaD_tilde, gradientD, hessianD = self.update_beta_w_graph(num_batch, num_station, lam, betaD_no_g, nu, G_net, G_poi, NT_CONVERGED, g_step)
        ...
        return count_uzo, count_uzd, count_uzt, betaO, betaD, betaT, gamma, theta, likelihood_mb, time_lkh
