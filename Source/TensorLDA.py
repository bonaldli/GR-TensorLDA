...
def d2_blhood(beta_old_exp, z, lam, beta_no_g_exp, mu, G_net, G_poi): # beta: R^(J * Vo), real value!
    ...
    while is_invertible(d2_beta) == False:
...
def update_beta_w_graph(beta_old, lam, beta_no_g, mu, G_net, G_poi): # beta_old: log-ed; beta_no_g: log-ed
...
def variationalInference(docs, u, gamma, phiO, phiD, phiT):
    ...
    #for i_infer in range(iterInference):
        ...
        bool_phi_converge = converge_paras(phiO_norm, phiD_norm, phiT_norm, phiO_norm_old, phiD_norm_old, phiT_norm_old, PARA_CONVERGED=0.001)
        
        phiO_norm_old = phiO_norm
        phiD_norm_old = phiD_norm
        phiT_norm_old = phiT_norm
        
        #MTRobot.sendtext(f'User {u} -- Likelihood: {likelihood_u:.3f}   Converged: {converged:.3f}')
        #MTRobot.sendtext(f'phiO: {phiO_norm:.3f}   phiD: {phiD_norm:.3f}   phiT: {phiT_norm:.3f}') 
...
def perplexity_overall(perO, perD, perT, wordcount_sum):
    wordcount_sum = sum(docs['wordcount'])
    np.log(perO*wordcount_sum)
...
#random.shuffle(indx)
#docs = docs_90.iloc[indx[0:1000]]
#docs_test = docs_90.iloc[indx[1000:docs_90.shape[0]]]
...
#for i_em in range(iterEM):
    ...
    bool_beta_converge = converge_paras(betaO_norm, betaD_norm, betaT_norm, betaO_norm_old, betaD_norm_old, betaT_norm_old, PARA_CONVERGED=0.001)
    
    betaO_norm_old = betaO_norm
    betaD_norm_old = betaD_norm
    betaT_norm_old = betaT_norm
    
    MTRobot.sendtext(f'End EM Iter {i_em} -- Likelihood: {likelihood:.3f}   Converged: {converged:.3f}')
    MTRobot.sendtext(f'betaO: {betaO_norm:.3f}   betaD: {betaD_norm:.3f}   betaT: {betaT_norm:.3f}') 
