...
from module_GR_TensorLDA_OnlineLearning_setting6 import GR_OnlineTensorLDA
...
save_dir = 'C:/Users/zlibn/Desktop/online_5k_s6_5e'
...
mb_inter = 10 # minibatch interval to fetch the result and plot
num_point = int(num_batch/mb_inter) # number of plot point within each epoch
likelihood_evolu_e = np.zeros((num_point*num_epoch, 2))#np.zeros((num_epoch, 2))
...
for lam_k, lam_v in enumerate(LAMBDA):
    ...
    for e in range(num_epoch):
        ...
        for i in range(num_batch):
            ...
            count_uzo, count_uzd, count_uzt, betaO, betaD, betaT, gamma, theta, likelihood_mb, time_lkh \
                = model.fit(i=i, docs_minibatch=docs_minibatch, lam=lam_v, mu=mu, nu=nu, G_net=G_net, G_poi=G_poi, \
                            dictionary_o=dictionary_o, dictionary_d=dictionary_d, dictionary_t=dictionary_t, idx_corpus_o=idx_corpus_o, idx_corpus_d=idx_corpus_d, idx_corpus_t=idx_corpus_t, num_user=num_user, num_station=num_station, num_time=num_time, \
                                    betaO=betaO, betaD=betaD, betaT=betaT, gamma=gamma, theta=theta)
            ...
            if (i+1) % mb_inter ==0:
                e1 = e * num_point + int((i+1) / mb_inter) - 1 
                ...
                MTRobot.sendtext(worker_idx, f' Finished {e1}-th checkpoint')
